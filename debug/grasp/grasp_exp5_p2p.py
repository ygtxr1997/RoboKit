...
class GraspPoseSender(object):
    def __init__(self, robot_ip="192.168.1.7"):
        ...
        self.bot = bot
        self.HOME = Transform(vec_mm=(0, 400, 450), euler_deg=(180, 0, 0))
    ...
    def run(self, objects):
        ...
        for obj in objects:
            ...
            # 构建抓取位姿
            T_gp = np.eye(4)
            T_gp[:3, :3] = np.array(obj["R"])
            T_gp[:3, 3] = np.array(obj["t"]) * 1000  # 转换为毫米
            print("T_gp: ", T_gp)
            ...
            # 靠近物体（离物体10cm）
            T_D10 = np.eye(4)
            T_D10[:3, 3] = np.array([0, 0, -210])  # 调整到更近的距离 #-230
