...
class MotionLib:
    ...
    def init_move(self,
                  no_of_goals):  # Initialises an array of the dictionary simple motion with the set number of goals
    ...
    def set_max_joint_velocity_acceleration(self, pos, velocity,
                                            acceleration):  # Sets the maximum join velocity and acceleration
    ...
    def start_motion(self, client, timeout):  # Starts the motion by sending the message to the action server
        ...
        message_ = {
            'motion_sequence': self.goal_info}  ## Creating a dictionary that looks the same as the simple motion
    ...
    def init_trajectory(self, joints, goals):
        self.joint_names = [f'j{x}' for x in
                            range(1, joints + 1)]  # Set the names of the joints from one to the number of joints
    ...
    def set_joint_velocity(self, goal, joint_num,
                           value):  # Set value for each value of joint velocity for the goal's motion
        self.trajectory_goals[goal]['velocities'][joint_num] = value

    def set_time_from_start(self, goal,
                            value):  # Set value for each value of time to finish motion for the goal's motion
