...
try:
  client = roslibpy.Ros(host='192.168.1.7', port=9090) # replace with your robots IP address
  client.run()
except:
...
try:
  ...
  ml.init_trajectory(6,1) # Number of joints and number of goals
  ...
  home_joints = [0, 0, 1.57, 0, 1.57, 3.14]
  for x in range(6):
    ml.set_joint_goal(0, x, home_joints[x])

  ml.set_time_from_start(0, 20)
