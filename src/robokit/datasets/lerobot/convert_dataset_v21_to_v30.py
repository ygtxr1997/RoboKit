...
def convert_episodes_metadata(root, new_root, episodes_metadata, episodes_video_metadata=None):
    ...
    # -------------------------------------------------------------------------
    # FIX: 遇到 (1, 1, 1) 的图像统计数据时，将其扩张为 (3, 1, 1) 以通过校验
    # -------------------------------------------------------------------------
    import numpy as np
    logging.info("Checking stats shapes and expanding (1,1,1) to (3,1,1) if necessary...")
    for ep_idx, feat_stats_dict in episodes_stats.items():
        for feat_key, stat_values in feat_stats_dict.items():
            for stat_name, stat_val in stat_values.items():
                # 检查是否为 numpy 数组且形状为 (1, 1, 1)
                if isinstance(stat_val, np.ndarray) and stat_val.shape == (1, 1, 1):
                    # 在通道维度重复3次
                    new_val = np.repeat(stat_val, 3, axis=0)
                    stat_values[stat_name] = new_val
                    print(f"[Warning] Expanded stats for episode_index {ep_idx}, feature '{feat_key}', "
                          "stat '{stat_name}' from (1,1,1) to (3,1,1).")
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # FIX: 手动分块写入，以绕过 write_episodes 的 100MB 限制
    # REPLACES: write_episodes(ds_episodes, new_root)
    # -------------------------------------------------------------------------
    # write_episodes(ds_episodes, new_root)
    logging.info("Saving episodes metadata (manual chunking to bypass 100MB limit)...")
    df_episodes = ds_episodes.to_pandas()

    # 估算每个分块的行数，使其保持在 ~90MB (留出安全缓冲)
    mem_usage = df_episodes.memory_usage(deep=True).sum()
    limit_bytes = 90 * 1024 * 1024

    if mem_usage > 0:
        rows_per_split = int(len(df_episodes) * limit_bytes / mem_usage)
    else:
        rows_per_split = len(df_episodes)
    rows_per_split = max(1, rows_per_split)

    chunk_idx = 0
    file_idx = 0

    for start_idx in range(0, len(df_episodes), rows_per_split):
        end_idx = min(start_idx + rows_per_split, len(df_episodes))
        sub_df = df_episodes.iloc[start_idx:end_idx]

        # 构造保存路径: meta/episodes/chunk-XXX/episodes_YYY.parquet
        save_dir = new_root / "meta/episodes" / f"chunk-{chunk_idx:03d}"
        save_dir.mkdir(parents=True, exist_ok=True)
        save_path = save_dir / f"episodes_{file_idx:03d}.parquet"

        sub_df.to_parquet(save_path, index=False)
